...
export async function checkSpam(userId, ip) {
  const hourlyKey = `lspd:spam:hourly:${userId}`;
  const burstKey = `lspd:spam:burst:${userId}`;
  const ipKey = `lspd:spam:ip:${ip || 'unknown'}`;
...
export async function resetSpam(userId, ip) {
  try {
    await kv.del(`lspd:spam:hourly:${userId}`);
    await kv.del(`lspd:spam:burst:${userId}`);
    if (ip) await kv.del(`lspd:spam:ip:${ip}`);
  } catch (error) {
